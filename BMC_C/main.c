...
/**
 * @brief Funktion die nur die Tasteneingaben a,A,c,C,g,G,t,T und 'ENTER' zul„sst, auf dem Bildschirm werden nur A C G T ausgegeben, nichts anderes
 *
 * @return char returned den eingegebenen Character
 */
char getDNABase(void)
{
    int eingabe_ASCII, breaker_var = 0; // Variablen fr die getch()-Eingabe und die Variable zum beenden der Schleife
    char ausgabe;
    while (breaker_var == 0)
    {
        eingabe_ASCII = getch();                        // Eingabe durch den Benutzer und umwandeln in ASCII-Wert
        if (eingabe_ASCII == 65 || eingabe_ASCII == 97) // Vergleich des ASCII-Werts, hier fr 'a' und 'A'
        {
